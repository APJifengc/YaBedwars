package com.yallage.yabedwars.edit;
...
public class EditGame implements Listener {
    ...
    private static void openMenu(Player player, Game game) {
        ...
        lore = new ArrayList<>();
        lore.add("");
        lore.add(Config.getLanguage("item.edit_game.lore.mix_players").replace("{players}", String.valueOf(game.getMinPlayers())));
        itemMeta.setLore(lore);
        itemStack.setItemMeta(itemMeta);
        ...
        YaBedwars.getInstance().getHolographicManager().displayGameLocation(player, game.getName());
    }
    ...
    private void alignLocation(Player player) {
        ...
        BigDecimal bd = new BigDecimal(String.valueOf(location.getX()));
        BigDecimal[] result = bd.divideAndRemainder(BigDecimal.valueOf(1L));
        double xd = Double.valueOf(result[1].toString());
        List<Double> list = Arrays.asList(-1.0D, -0.5D, 0.0D, 0.5D, 1.0D);
        ...
        bd = new BigDecimal(String.valueOf(location.getZ()));
        result = bd.divideAndRemainder(BigDecimal.valueOf(1L));
        double zd = Double.valueOf(result[1].toString());
        list = Arrays.asList(-1.0D, -0.5D, 0.0D, 0.5D, 1.0D);
        a = Math.abs(list.get(0) - zd);
    ...
    private void alignAngle(Player player) {
        ...
        a = Math.abs(list.get(0) - pitch);
        double npitch = list.get(0);
        for (Double double_ : list) {
            double j = Math.abs(double_ - pitch);
            if (j < a) {
                a = j;
                npitch = double_;
            }
    ...
    private static void openEditTeamsMenu(Player player, Game game) {
        ...
        List<TeamColor> colors = new ArrayList<>(Arrays.asList(TeamColor.values()));
        ...
        for (Team team : game.getTeams().values()) {
            ...
            List<String> lore = new ArrayList<>();
            lore.add("§1");
            lore.add(Config.getLanguage("item.edit_game.lore.max_players").replace("{players}", String.valueOf(team.getMaxPlayers())));
            lore.add("");
    ...
    private void onPacketSending() {
        ...
        PacketAdapter packetAdapter = new PacketAdapter(YaBedwars.getInstance(), ListenerPriority.HIGHEST,
            ...
            public void onPacketReceiving(PacketEvent e) {
                ...
                if (e.getPacketType().equals(PacketType.Play.Client.WINDOW_CLICK) && packet.getIntegers().read(0) == 0) {
                    ...
                    if (packet.getIntegers().read(1) == 2 &&
                        ...
                        if (itemMeta.getDisplayName() != null) {
                            ...
                            if (args.length == 5 && args[1].equalsIgnoreCase("addteam")) {
                                ...
                                if (game != null)
                                    YaBedwars.getInstance().getHolographicManager().displayGameLocation(player,
                                            game.getName());
                            }
                        }
                    }
    ...
    private static void setAnvilItem(Player player, String game, String str) {
        ...
        try {
            ...
            itemMeta.setLore(Collections.singletonList(lore.substring(0, lore.length() - 1)));
            itemStack.setItemMeta(itemMeta);
