...
public class Config {
  ...
  public static HashMap<Material, Integer> resources = new HashMap<>();
  ...
  public static void loadConfig() {
    ...
    if (!file_config.exists()) {
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &b配置文件不存在,正在创建...");
      BedwarsXP.plugin.saveResource("config.yml", true);
    } 
    yaml_config = YamlConfiguration.loadConfiguration(file_config);
    if (yaml_config.getInt("ConfigVersion") < configVersion) {
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &4您的配置文件版本过老无法加载");
      file_config.renameTo(new File("plugins/BedwarsXP/config.bak.yml"));
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &c已备份您的配置文件为 [config.bak.yml] ,开始初始化新版本配置文件");
      BedwarsXP.plugin.saveResource("config.yml", true);
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a配置文件初始化完成,继续加载配置...");
      file_config = new File("plugins/BedwarsXP/config.yml");
      yaml_config = YamlConfiguration.loadConfiguration(file_config);
    } 
    ...
    if (addResShop)
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a已启用经验兑换资源商店"); 
    deathCost = yaml_config.getInt("DeathCostXP", 0) / 100.0D;
    BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a死亡扣除 " + (deathCost * 100.0D) + "% 经验");
    deathDrop = yaml_config.getInt("DeathDropXP", 0) / 100.0D;
    BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a死亡掉落经验" + ((deathDrop == 0.0D) ? "已关闭" : ("占扣除经验 " + (deathDrop * 100.0D) + "%")));
    maxXP = yaml_config.getInt("MaxXP");
    maxXPMessage = yaml_config.getString("MaxXPMessage").replaceAll("&", "§").replaceAll("§§", "&");
    BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a最大经验限制已" + ((maxXP == 0) ? " 关闭" : ("设置为 " + maxXP)));
    fullXPBedwars = yaml_config.getBoolean("Full_XP_Bedwars");
    if (fullXPBedwars)
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a完全经验起床模式已启动"); 
    BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &a开始加载资源价值数据");
    ...
    for (String key : resourceSection.getKeys(false)) {
      ...
      for (Map<String, Object> resource : resourceList) {
        ...
        resources.put(mat, Integer.valueOf(xp));
    ...
    if (!file_enabledGames.exists()) {
      BedwarsXP.sendConsoleMessage("§6§l[BedwarsXP] &c注意,在新版本中你需要手动使用/bwxp enable来启用游戏的经验起床模式");
      BedwarsXP.plugin.saveResource("enabledGames.yml", true);
    } 
